    ...
    if (fs.existsSync(decisionsPath)) {
      ...
      for (const decision of decisions) {
        if (decision.decision === "accept" || decision.decision === "edit") {
          ...
          if (fact) {
            const value = decision.decision === "edit" ? decision.editedValue : fact.proposedValue;
            if (!isTrivial(decision.jsonPath, value)) {
              accepted_nontrivial_count++;
              const anchors = fact.repoSources || [];
              if (anchors.length > 0) {
    ...
    try {
      ...
      for (const node of leafNodes) {
        if (!isTrivial(node.path, node.value)) {
          ...
          if (anchoredPaths.has(node.path)) {
