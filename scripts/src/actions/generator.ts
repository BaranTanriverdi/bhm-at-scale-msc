...
async function main(): Promise<void> {
  ...
  if (isFirstRun) {
    ...
    logger.info("Full scan selected files", { count: changedFiles.length });
  } else {
    ...
    changedFiles = incremental.changed;
    logger.info("Computed incremental file set", { changed: changedFiles.length });
  }
