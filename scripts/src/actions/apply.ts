...
async function main(): Promise<void> {
  ...
  while (loopCount < MAX_LOOPS) {
      ...
      acceptedPatch = filterPatchByDecisions(proposal, filteredFacts, currentCard as object);
  ...
  if (failedFacts.length > 0) {
    ...
    // Also remove decisions for applied paths (cleanup)
    const pendingDecisions = newDecisions.filter((d) => !appliedPaths.has(d.jsonPath));

    await writeTextFile(join(PROPOSALS_DIR, `${args.runId}.decisions.json`), JSON.stringify(pendingDecisions, null, 2));

  } else {
    await cleanupProposalFiles(args.runId);
  }
...
function filterPatchByDecisions(proposal: Proposal, acceptedFacts: Fact[], currentCard: object): Operation[] {
